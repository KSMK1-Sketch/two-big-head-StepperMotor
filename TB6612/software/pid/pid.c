...
void pid_control()
{
	...
	// 2.获取当前速度
	if (motorA_dir){motorA.now = -Encoder_Getcount(1);}else{motorA.now = Encoder_Getcount(1);}
	if (motorB_dir){motorB.now = -Encoder_Getcount(2);}else{motorB.now = Encoder_Getcount(2);}
	/* 编码器计数值清零 */
	Encoder_Clearcount();
	// 3.输入PID控制器进行计算
	pid_cal(&motorA);
