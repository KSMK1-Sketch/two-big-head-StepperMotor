...
void GROUP1_IRQHandler(void)
{
    ...
    // 获取中断信号
    gpio_interrup = DL_GPIO_getEnabledInterruptStatus(ENCODER_PORT, ENCODER_E1A_PIN | ENCODER_E1B_PIN | ENCODER_E2A_PIN | ENCODER_E2B_PIN | ENCODER_E3A_PIN | ENCODER_E3B_PIN | ENCODER_E4A_PIN | ENCODER_E4B_PIN);
    ...
    // encoderC
    if ((gpio_interrup & ENCODER_E3A_PIN) == ENCODER_E3A_PIN)
    {
        if (!DL_GPIO_readPins(ENCODER_PORT, ENCODER_E3B_PIN))
        {
    ...
    else if ((gpio_interrup & ENCODER_E3B_PIN) == ENCODER_E3B_PIN)
    {
        if (!DL_GPIO_readPins(ENCODER_PORT, ENCODER_E3A_PIN))
        {
            Encoder_C.Should_Get_Encoder_Count++;
        }
        else
    ...
    // encoderD
    if ((gpio_interrup & ENCODER_E4A_PIN) == ENCODER_E4A_PIN)
    {
        if (!DL_GPIO_readPins(ENCODER_PORT, ENCODER_E4B_PIN))
        {
    ...
    else if ((gpio_interrup & ENCODER_E4B_PIN) == ENCODER_E4B_PIN)
    {
        if (!DL_GPIO_readPins(ENCODER_PORT, ENCODER_E4A_PIN))
        {
            Encoder_D.Should_Get_Encoder_Count++;
        }
        else
    ...
    DL_GPIO_clearInterruptStatus(ENCODER_PORT, ENCODER_E1A_PIN | ENCODER_E1B_PIN | ENCODER_E2A_PIN | ENCODER_E2B_PIN | ENCODER_E3A_PIN | ENCODER_E3B_PIN | ENCODER_E4A_PIN | ENCODER_E4B_PIN);
}
